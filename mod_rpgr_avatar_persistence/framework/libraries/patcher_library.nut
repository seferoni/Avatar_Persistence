::AP.Patcher <-
{
	...
	{
		if (::AP.Manager.isModernHooksInstalled())
		{
			::AP.Integrations.ModernHooks.hook(_path, _function);
			return;
		}

		::AP.Integrations.ModdingScriptHooks.hook(this.formatPath(_path), _function);
	}
	...
	function hookBase( _path, _function )
	{
		if (::AP.Manager.isModernHooksInstalled())
		{
			::AP.Integrations.ModernHooks.hookBase(_path, _function);
			return;
		}

		::AP.Integrations.ModdingScriptHooks.hookBase(this.formatPath(_path), _function);
	}
	...
	function hookTree( _path, _function )
	{
		if (::AP.Manager.isModernHooksInstalled())
		{
			::AP.Integrations.ModernHooks.hookTree(_path, _function);
			return;
		}

		::AP.Integrations.ModdingScriptHooks.hookTree(this.formatPath(_path), _function);
	}
	...
	function wrap( _object, _methodName, _function, _procedure = "overrideReturn" )
	{
		...
		_object.rawset(_methodName, function( ... )
		{
			...
			local originalMethod = cachedMethod == null ? ::AP.Patcher.getMethodFromParent(this, parentName, _methodName) : cachedMethod;

			if (!::AP.Patcher.validateParameters(originalMethod, vargv))
			{
				::AP.Standard.log(format("An invalid number of parameters were passed to %s, aborting wrap procedure.", _methodName), true);
				return;
			}

			local argumentsArray = ::AP.Patcher.prependContextObject(this, vargv);
			return ::AP.Patcher[_procedure](this, _function, originalMethod, argumentsArray);
		});
	}
};