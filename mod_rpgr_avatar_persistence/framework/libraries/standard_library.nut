::AP.Standard <-
{
	...
	function constructEntry( _icon, _text, _parentArray = null )
	{
		...
		if (_icon != null)
		{
			entry.icon <- ::AP.Database.getIcon(_icon);
		}
	...
	{
		local flagValue = _object.getFlags().get(format("%s.%s", ::AP.ID, _string));
	...
	{
		local flagValue = _object.getFlags().getAsInt(format("%s.%s", ::AP.ID, _string));
	...
	function getParameter( _parameterID )
	{
		...
		{
			return ::AP.Interfaces.MSU.ModSettings.getSetting(_parameterID).getValue();
		}

		local parameters = ::AP.Database.getSettingParameters();
	...
	{
		return ::AP.Database.getField("Generic", "Procedures");
	}
